  ...
  return (
    <div className="px-[15px] md:px-[32px] lg:px-[50px] py-[20px] lg:py-[36px] flex flex-col w-full mx-auto max-w-[1200px]">
      <div className="flex justify-between items-baseline mb-[40px]">
        <h1 className="text-[24px] md:text-[40px] text-customDarkGray leading-[32px] md:leading-[52px] tracking-[0.6px] mb-[20px]">
          {t('cart.cart_title')}
        </h1>
        <Link href="/products">
          <p className="text-[10px] md:text-[12px] text-customGray tracking-[0.6px] text-center underline underline-offset-4">
            {t('cart.continue_shopping')}
          </p>
        </Link>
      </div>
      {!cart.length ? (
        <p className="text-[14px] md:text-[20px] text-customGray leading-[24px] tracking-[0.6px]">
          {t('cart.cart_empty')}
        </p>
      ) : (
        <>
